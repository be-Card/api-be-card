...
def get_puntos_venta(session: Session = Depends(get_session)):
    puntos = session.exec(select(PuntoVenta).order_by(PuntoVenta.nombre)).all()
...
def get_equipos(
    ...
    equipos = EquipoService.get_equipos_with_details(session)
...
def cambiar_cerveza_equipo(
    ...
    try:
        ...
        logger.warning(f"Validation error changing beer: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception:
