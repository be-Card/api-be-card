...
async def get_client_detail(
    ...
    """
    result = ClientService.get_client_detail(session, client_id)
    if not result:
...
async def update_client(
    ...
    result = ClientService.update_client(session, client_id, update_dict)
    if not result:
        raise HTTPException(status_code=404, detail=f"Cliente con ID {client_id} no encontrado")
...
async def toggle_client_status(
    ...
    """
    result = ClientService.toggle_client_status(session, client_id, status_data.reason)
    if not result:
        raise HTTPException(status_code=404, detail=f"Cliente con ID {client_id} no encontrado")
...
async def get_client_loyalty_history(
    ...
    user_id = session.exec(select(Usuario.id).where(Usuario.id_ext == client_id)).first()
    if not user_id:
        raise HTTPException(status_code=404, detail=f"Cliente con ID {client_id} no encontrado")
...
async def get_client_rewards(
    ...
    user = session.exec(select(Usuario).where(Usuario.id_ext == client_id)).first()
    if not user:
        raise HTTPException(status_code=404, detail=f"Cliente con ID {client_id} no encontrado")
...
async def redeem_client_reward(
    ...
    try:
        return ClientService.redeem_reward(session, client_id, premio_id)
